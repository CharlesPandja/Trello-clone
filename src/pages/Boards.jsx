  ...
  return (
    <main className="w-3/4 min-h-screen bg-zinc-800 overflow-hidden text-stone-200">
      <div className="mt-30 ml-12">
        ...
        <div className="flex flex-wrap justify-left items-center gap-4 text-white font-semibold text-base cursor-pointer">
          {tableauSidebar ? tableauSidebar.map(element => <Link to={element.titre}><div key={element.id} className={`${element.backgroundColor} px-2 py-2 rounded-sm w-50 h-25`}>
            <p>{element.titre}</p>
